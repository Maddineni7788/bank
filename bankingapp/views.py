...
def create_transaction(request):
    ...
    if request.method == "POST":
        ...
        return redirect('bankingapp:user_success')
...
def withdrawal(request):
    ...
    if request.method == "POST":
        ...
        send_mail(subject,f'You have withdrawn {withdrawal_amount} {currency} from your account {account_number}.',settings.DEFAULT_FROM_EMAIL,[email])
        # Redirect to a success page or another relevant view
        return redirect('bankingapp:success')
...
def deposit(request):
    ...
    if request.method == "POST":
        ...
        send_mail(subject,f'You have withdrawn {deposit_amount} {currency} from your account {account_number}.',settings.DEFAULT_FROM_EMAIL,[email])
        # Redirect to a success page or another relevant view
        return redirect('bankingapp:success')
...
def user_reg(request):
    if request.method == "POST":
        ...
        if password == password2:
            ...
            messages.error(request, 'Passwords do not match')
            return redirect('bankingapp:user_reg')
    else:
...
def user_login(request):
    if request.method=="POST":
        ...
        if user is not None:
            auth.login(request,user)
            return redirect('bankingapp:user_dashboard')
        else:
            messages.info(request,'Invalid user credentials')
            return redirect('bankingapp:user_login')
    else:
...
def create_account(request):
    if request.method == 'POST':
        ...
        initial_deposit = request.POST.get('initialDeposit')

        # Perform additional server-side validation and processing here
        ...
        b.save()
        subject = 'Account Details Notification'
        send_mail(subject,f'You have created a account and The Account Number is: {account_number}.',settings.DEFAULT_FROM_EMAIL,[email])
        return redirect('bankingapp:success')
...
def search_accounts(request):
    ...
    return render(request, 'search_accounts.html', {'query': query, 'accounts': accounts})
